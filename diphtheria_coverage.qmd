...
```{r echo=FALSE}
# preprocess data
cov_by_perma <- readRDS("_fr_registry/data/dip_coverage_by_perma.rds")
cov_by_reg <- readRDS("_fr_registry/data/dip_coverage_by_reg.rds")

cov_by_perma <- cov_by_perma %>% 
  mutate(
    province = case_when(
  ...
  rename(eval_year = curr_eval_year) %>% 
  mutate(
    eval_date = as.Date(paste0(eval_year, "-12-31")),
    .before = "eval_year"
  )
...
write_rds(cov_by_perma, "data/dip_coverage_by_perma.rds")
write_rds(cov_by_reg, "data/dip_coverage_by_reg.rds")
```
...
+===========================+====================================================================================================================================+
| `eval_date`               | Date of evaluation (i.e. the date for which coverage is computed)                                                                  |
+---------------------------+------------------------------------------------------------------------------------------------------------------------------------+
| `eval_year`               | Year of evaluation (i.e. year in `eval_date`)                                                                                      |
+---------------------------+------------------------------------------------------------------------------------------------------------------------------------+
